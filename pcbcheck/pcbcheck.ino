...
void printReg(unsigned char address){
  ...
  for(int i=31-8; i>=8; i--){ //print only the middle 16 bits
...
void initTMAG5170(){
  ...
  regSend(writeReg, TEST_CONFIG, CRC_DIS_CRCdisabled);
  ...
  for (byte address = 0x0; address < 0x15; address++){
    ...
    regSend(writeReg, address, data);
  }
  regSend(writeReg, DEVICE_CONFIG, DeviceStart);
}
...
void regSend(unsigned char RW, unsigned char address, unsigned int data){
  ...
  sendBuffer[3] = 0x07; //CRC specifically for disabling CRC
